...
export class PlantFamilyScene extends Phaser.Scene {
  ...
  private illustrationContainer: Phaser.GameObjects.Container | null = null;
  ...
  private createFamilyCard(
    ...
    const iconText = this.add.text(x - width / 2 + 70, y, family.icon, {
      font: '40px Arial'
    }).setOrigin(0.5);
    items.push(iconText);

    const nameText = this.add.text(x - width / 2 + 150, y - 40, `${family.familyName} · ${family.genusName}`, {
      font: 'bold 22px Arial',
      color: hasUnlocked ? '#ffffff' : '#888888'
    }).setOrigin(0, 0.5);
    items.push(nameText);

    const descText = this.add.text(x - width / 2 + 150, y - 5, family.description, {
      font: '14px Arial',
    ...
    const countText = this.add.text(x - width / 2 + 150, y + 35, `收藏: ${unlockedCount}/${totalCount}`, {
      font: 'bold 16px Arial',
      color: '#4caf50'
    }).setOrigin(0, 0.5);
    ...
    items.push(progressFill);

    const percentText = this.add.text(x + width / 2 - 60, y + 61, `${progressPercent}%`, {
      font: 'bold 14px Arial',
      color: '#ffffff'
    }).setOrigin(1, 0.5);
    items.push(percentText);
    ...
    if (family.isLimited) {
      ...
      items.push(limitedBadge);

      const limitedLabel = this.add.text(x + width / 2 - 45, y - height / 2 + 28, '限定', {
        font: 'bold 12px Arial',
        color: '#ffffff'
      }).setOrigin(0.5);
      items.push(limitedLabel);
    }
    ...
    if (availableRewards.length > 0) {
      ...
      items.push(rewardBadge);

      const rewardMark = this.add.text(x + width / 2 - 25, y - height / 2 + 25, '!', {
        font: 'bold 18px Arial',
        color: '#1a1a2e'
      }).setOrigin(0.5);
      items.push(rewardMark);
    }
  ...
  private showFamilyDetail(family: PlantFamily): void {
    ...
    c.add(this.add.text(85, 170, family.icon, {
      font: '50px Arial'
    }).setOrigin(0, 0.5));

    c.add(this.add.text(160, 160, `${family.familyName} · ${family.genusName}`, {
      font: 'bold 26px Arial',
      color: '#ffffff'
    }).setOrigin(0, 0.5));

    c.add(this.add.text(160, 195, family.featureDescription, {
      font: '14px Arial',
      color: '#cccccc',
      wordWrap: { width: 500 }
    }).setOrigin(0, 0.5));
    ...
    c.add(this.add.text(605, 170, `${progressPercent}%`, {
    ...
    c.add(this.add.text(60, currentY, '🌱 家族成员', {
      font: 'bold 22px Arial',
      color: '#e94560'
    }).setOrigin(0, 0.5));
    currentY += 30;

    family.specimenIds.forEach((specimenId) => {
      ...
      if (isUnlocked) {
        ...
        c.add(this.add.text(150, currentY + 25, specimen?.name || '未知', {
          font: 'bold 18px Arial',
          color: '#ffffff'
        }).setOrigin(0, 0.5));

        c.add(this.add.text(150, currentY + 55, `${specimen?.family} · ${specimen?.genus}`, {
          font: '13px Arial',
          color: '#aaaaaa'
        }).setOrigin(0, 0.5));
        ...
        if (galleryItem) {
          ...
          if (progress) {
            this.drawMiniStars(640, currentY + 40, progress.stars, c);
          }
        }
      } else {
        const lockImg = this.add.image(100, currentY + 40, 'lock').setScale(0.8);
        c.add(lockImg);

        c.add(this.add.text(150, currentY + 40, '???', {
          font: 'bold 18px Arial',
          color: '#666666'
        }).setOrigin(0, 0.5));
      }
    ...
    c.add(this.add.text(60, currentY, '🎁 套装奖励', {
      font: 'bold 22px Arial',
      color: '#e94560'
    }).setOrigin(0, 0.5));
    currentY += 30;
    ...
    family.rewards.forEach((reward) => {
      ...
      if (isClaimed) {
        ...
        claimBtn.on('pointerup', () => {
          this.claimReward(family.id, reward.id, c);
        });
      }
    ...
    c.add(this.add.text(60, currentY, '🖼️ 专属插画', {
      font: 'bold 22px Arial',
      color: '#e94560'
    }).setOrigin(0, 0.5));
    currentY += 30;
    ...
    c.add(illustrationBg);

    if (illustrationUnlocked) {
      const illustrationColors = [family.primaryColor, family.secondaryColor, family.accentColor];
      const art = this.createIllustrationArt(375, currentY + 100, 500, 160, illustrationColors);
      c.add(art);

      c.add(this.add.text(375, currentY + 180, family.illustrationTitle, {
        font: 'bold 18px Arial',
        color: '#ffffff'
      }).setOrigin(0.5));
      ...
      c.add(this.add.text(375, currentY + 220, '查看大图', {
      ...
      const lockImg = this.add.image(375, currentY + 80, 'lock').setScale(1.5);
      c.add(lockImg);

      c.add(this.add.text(375, currentY + 140, '收集进度达到100%解锁', {
        font: 'bold 16px Arial',
        color: '#888888'
      }).setOrigin(0.5));

      c.add(this.add.text(375, currentY + 170, `当前进度: ${progressPercent}%`, {
        font: '14px Arial',
        color: '#666666'
      }).setOrigin(0.5));
    }
    ...
    c.add(this.add.text(375, 1207, '返回列表', {
  ...
  private showFullIllustration(family: PlantFamily): void {
    this.destroyIllustrationContainer();

    const c = this.add.container(0, 0);
    this.illustrationContainer = c;
    ...
    c.add(this.add.text(375, 150, family.illustrationTitle, {
    ...
    c.add(this.add.text(375, 1060, family.illustrationDescription, {
      font: '16px Arial',
      color: '#ffffff',
      align: 'center',
      wordWrap: { width: 580 }
    ...
    c.add(this.add.text(375, 1180, '关闭', {
      font: 'bold 22px Arial',
      color: '#ffffff'
    }).setOrigin(0.5));
  ...
  private claimReward(familyId: string, rewardId: number, container: Phaser.GameObjects.Container): void {
    ...
    if (result.success && result.reward) {
      const n = this.add.container(0, 0);
      ...
      n.add(this.add.text(375, 635, '🎉 奖励领取成功！', {
        font: 'bold 24px Arial',
        color: '#ffffff'
      }).setOrigin(0.5));

      n.add(this.add.text(375, 675, `获得: ${result.reward.name}`, {
        font: '18px Arial',
        color: '#ffd700'
      }).setOrigin(0.5));
  ...
  private destroyDetailContainer(): void {
  ...
  private destroyIllustrationContainer(): void {
    if (this.illustrationContainer) {
      this.illustrationContainer.destroy();
